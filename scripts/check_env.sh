...
# are visible BEFORE you configure.  Run:  bash scripts/check_env.sh
set -u
...
if have nvidia-smi; then
  ...
  if [ -n "${CC:-}" ]; then
    ...
    say "compute capability" "nvidia-smi too old to report it; look up your GPU's value"
  fi
else
  say "GPU" "nvidia-smi not found — you are probably on a login node with no GPU."
  echo
  echo "  If building here, you MUST pass the compute-node GPU's architecture, e.g."
